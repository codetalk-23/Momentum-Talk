...
<a href="https://www.raycast.com/mattiacolombomc/Momentum" title="Install Momentum Raycast Extension"><img src="https://www.raycast.com/mattiacolombomc/Momentum/install_button@2x.png?v=1.1" height="64" style="height: 64px;" alt="Install Momentum Raycast Extension" /></a>

Control Momentum from [Raycast](https://www.raycast.com) — start/stop recording, browse transcript history, manage dictionary, switch models and languages.

[Source](https://github.com/mattiacolombomc/raycast-Momentum) · by [@mattiacolombomc](https://github.com/mattiacolombomc)
...
```bash
Momentum --toggle-transcription    # Toggle recording on/off
Momentum --toggle-post-process     # Toggle recording with post-processing on/off
Momentum --cancel                  # Cancel the current operation
```

**Startup flags:**

```bash
Momentum --start-hidden            # Start without showing the main window
Momentum --no-tray                 # Start without the system tray icon
Momentum --debug                   # Enable debug mode with verbose logging
Momentum --help                    # Show all available flags
```
...
The following are recommendations for running Momentum on your own machine. If you don't meet the system requirements, the performance of the application may be degraded. We are working on improving the performance across all kinds of computers and hardware.
...
- Collect anonymous usage data to help improve Momentum
...
Momentum release artifacts are signed with Tauri's updater signature format. The public key is stored in [`src-tauri/tauri.conf.json`](src-tauri/tauri.conf.json) under `plugins.updater.pubkey`.

To verify a release manually, set `ARTIFACT` to the filename you downloaded, save the `pubkey` value from `src-tauri/tauri.conf.json` to `Momentum.pub.b64`, then decode the public key and matching `.sig` file from base64 and verify the artifact with `minisign`:
...
pub = pathlib.Path("Momentum.pub.b64").read_text().strip()
pathlib.Path("Momentum.pub").write_bytes(base64.b64decode(pub))
...
- **macOS**: `~/Library/Application Support/com.pais.Momentum/`
- **Windows**: `C:\Users\{username}\AppData\Roaming\com.pais.Momentum\`
- **Linux**: `~/.config/com.pais.Momentum/`
...
```bash
# macOS/Linux
mkdir -p ~/Library/Application\ Support/com.pais.Momentum/models

# Windows (PowerShell)
New-Item -ItemType Directory -Force -Path "$env:APPDATA\com.pais.Momentum\models"
```
...
- Small (487 MB): `https://blob.Momentum.computer/ggml-small.bin`
- Medium (492 MB): `https://blob.Momentum.computer/whisper-medium-q4_1.bin`
- Turbo (1600 MB): `https://blob.Momentum.computer/ggml-large-v3-turbo.bin`
- Large (1100 MB): `https://blob.Momentum.computer/ggml-large-v3-q5_0.bin`

**Parakeet Models (compressed archives):**

- V2 (473 MB): `https://blob.Momentum.computer/parakeet-v2-int8.tar.gz`
- V3 (478 MB): `https://blob.Momentum.computer/parakeet-v3-int8.tar.gz`
...
- After placing the files, restart Momentum to detect the new models
...
3. Restart Momentum to discover the new model
...
1. **Check existing issues** at [github.com/cjpais/Momentum/issues](https://github.com/cjpais/Momentum/issues)
2. **Fork the repository** and create a feature branch
3. **Test thoroughly** on your target platform
4. **Submit a pull request** with clear description of changes
5. **Join the discussion** - reach out at [contact@Momentum.computer](mailto:contact@Momentum.computer)
...
<div align="center">
  ...
  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
  <a href="https://boltai.com?utm_source=Momentum">
...
- **[Momentum CLI](https://github.com/cjpais/Momentum-cli)** - The original Python command-line version
- **[Momentum.computer](https://Momentum.computer)** - Project website with demos and documentation
